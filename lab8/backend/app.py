...
def post_user():
    ...
    if not data or 'id' not in data or 'first_name' not in data or 'user_group' not in data:
        return jsonify({"message": "Missing required fields: id, first_name, user_group"}), 400
...
def put_user(user_id):
    data = request.get_json()
    if not data or not data.get('first_name') or data.get('user_group') is None:
        return jsonify({"message": "Missing required fields: first_name, user_group"}), 400
